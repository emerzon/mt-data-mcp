...
def _channel_width_expansion_ratio(
    ...
    last = max(5, int(k))
    recent = (
        float(np.mean(width[-last:])) if width.size >= last else float(np.mean(width))
    )
    past_win = max(20, 2 * int(k))
    prev_win = (
        width[-past_win:-last] if n > past_win else width[: max(1, width.size // 2)]
    )
...
def detect_trend_lines(
    ...
    for side, piv in (("high", peaks), ("low", troughs)):
        if piv.size >= max(3, cfg.min_touches):
            ...
            ys = c[idxs]
            slope, intercept, r2 = (
                _fit_line_robust(xs, ys, cfg)
                if cfg.use_robust_fit
                else _fit_line(xs, ys)
            )
            ...
            tl_dir = (
                "Ascending"
                if slope > cfg.max_flat_slope
                else ("Descending" if slope < -cfg.max_flat_slope else "Horizontal")
            )
            name = f"{tl_dir} Trend Line"

            recent_i = n - 1
            recent_touches = _count_recent_touches(
                line_vals, c, tol_abs, confirm_lookback
            )
...
def detect_channels(
    ...
    if not _boundaries_are_ordered(
        upper, lower, start_idx=channel_start, end_idx=n - 1
    ):
        return ch_results
    ...
    if (
        ...
        and not converging
        and not widening
        and touches >= cfg.min_channel_touches
    ):
        ...
        recent_hits = _count_recent_touches(
            upper, c, tol_abs, confirm_lookback
        ) + _count_recent_touches(lower, c, tol_abs, confirm_lookback)
        breakout_direction, breakout_index = _find_recent_breakout(
            c,
            upper=upper,
            lower=lower,
        ...
        if breakout_index is not None:
            conf = min(1.0, float(conf) + 0.06)
        end_i = int(
            breakout_index if breakout_index is not None else max(ih[-1], il[-1])
        )
        ...
        details = {
            ...
            "bias": "bullish"
            if name == "Ascending Channel"
            else "bearish"
            if name == "Descending Channel"
            else "neutral",
        }
