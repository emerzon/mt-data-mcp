...
def build_target_series(
    ...
    if not target_spec or not isinstance(target_spec, dict):
        if str(quantity).strip().lower() == "return":
            log_prices = np.log(
                np.maximum(df[base_col].astype(float).to_numpy(), 1e-12)
            )
    ...
    if ts_inds:
        try:
            specs = (
                _parse_ti_specs_util(str(ts_inds))
                if isinstance(ts_inds, str)
                else ts_inds
            )
    ...
    if base_name in df.columns:
        ...
        if y_base is None:
            raise ValueError(
                f"Base column '{base_name}' not found and not a recognized alias"
            )
