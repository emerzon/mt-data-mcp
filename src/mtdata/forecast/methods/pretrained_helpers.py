...
def extract_context_window(
    series: np.ndarray, context_length: int, n: int, dtype: np.dtype = np.float64
) -> np.ndarray:
    """
...
def validate_and_clean_data(
    data: np.ndarray, method_name: str = "forecast"
    ...
    if not np.any(np.isfinite(cleaned_data)):
        return np.array(
            []
        ), f"{method_name} error: cleaned context contains no finite values"
...
def extract_forecast_values(
    ...
    try:
        ...
        # Try median if requested and mean not available
            ...
            do_median
            and hasattr(forecast_obj, "median")
            and forecast_obj.median is not None
        ):
        ...
        # Fallback to samples if available
            ...
            return (
                None,
                f"{method_name} error: no forecast values available (no mean, median, or samples)",
            )
    except Exception as extract_ex:
        return (
            None,
            f"{method_name} error: failed to extract forecast values: {extract_ex}",
        )
...
def adjust_forecast_length(
    values: np.ndarray, fh: int, method_name: str = "forecast"
) -> np.ndarray:
    """
...
def process_quantile_levels(
    quantiles: Optional[List[Union[str, float, int]]], method_name: str = "forecast"
) -> Optional[List[float]]:
    """
...
def build_params_used(
    ...
    if quantiles_dict:
        params_used["quantiles"] = sorted(
            list(quantiles_dict.keys()), key=lambda x: float(x)
        )
...
def validate_required_params(
    params: Dict[str, Any], required_keys: List[str], method_name: str = "forecast"
