    ...
    PARAMS: List[Dict[str, Any]] = [
        {
            "name": "alpha",
            "type": "float",
            "description": "SES smoothing factor (default: 0.2).",
        },
    ]
    ...
    def forecast(
        ...
        **kwargs,
        ...
        return ForecastResult(
            forecast=f_vals, params_used={"alpha": alpha, "trend_slope": b}
        )
...
class FourierOLSMethod(ClassicalMethod):
    PARAMS: List[Dict[str, Any]] = [
        {"name": "seasonality", "type": "int", "description": "Seasonal period (m)."},
        {
            "name": "terms",
            "type": "int",
            "description": "Number of Fourier harmonics (default: 3).",
        },
        {
            "name": "trend",
            "type": "bool",
            "description": "Include linear trend (default: True).",
        },
    ]
...
def forecast_drift(
    series: np.ndarray, fh: int, n: Optional[int] = None
...
def forecast_seasonal_naive(
    series: np.ndarray, fh: int, m: int
) -> Tuple[np.ndarray, Dict[str, Any]]:
...
def forecast_theta(
    series: np.ndarray, fh: int, alpha: float = 0.2
) -> Tuple[np.ndarray, Dict[str, Any]]:
    res = ForecastRegistry.get("theta").forecast(
...
def forecast_fourier_ols(
    series: np.ndarray, fh: int, m: Optional[int], K: Optional[int], trend: bool = True
) -> Tuple[np.ndarray, Dict[str, Any]]:
    res = ForecastRegistry.get("fourier_ols").forecast(
        pd.Series(series), fh, m or 0, {"terms": K, "trend": trend}
    )
    return res.forecast, res.params_used
