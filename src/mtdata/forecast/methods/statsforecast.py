        ...
        **kwargs,
        ...
        try:
            with warnings.catch_warnings():
                ...
                sf = StatsForecast(
                    models=[model], freq=1
                )  # freq=1 for integer index fallback
                if X_df is not None:
            ...
            if Yf.empty:
                raise RuntimeError(
                    "StatsForecast output missing rows for unique_id='ts'"
                )
            ...
            clean_params = {
                k: v for k, v in clean_params.items() if k not in internal_keys
            }
            params_used = {"seasonality": seasonality, **clean_params}

            return ForecastResult(
                forecast=f_vals, ci_values=ci_values, params_used=params_used
            )
...
class GenericStatsForecastMethod(StatsForecastMethod):
    ...
    PARAMS: List[Dict[str, Any]] = [
        {
            "name": "model_name",
            "type": "str",
            "description": "StatsForecast model class name.",
        },
        {
            "name": "season_length",
            "type": "int",
            "description": "Season length (auto if omitted).",
        },
    ]
    ...
    def _get_model(self, seasonality: int, params: Dict[str, Any]):
        ...
        class_names = [
            m for m in public_names if inspect.isclass(getattr(models, m, None))
        ]
        ...
        if target not in available:
            raise ValueError(
                f"Unknown StatsForecast model: {model_name}. Available: {list(available.keys())}"
            )
