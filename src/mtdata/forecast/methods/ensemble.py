...
def _record_dispatch_error(
    dispatch_method: Any, method_name: str, exc: BaseException
) -> None:
    try:
        setattr(
            dispatch_method,
    ...
    def forecast(
        ...
        **kwargs,
        ...
        if isinstance(base_methods_in, str):
            base_methods = [
                m.strip().lower() for m in base_methods_in.split(",") if m.strip()
            ]
        elif isinstance(base_methods_in, (list, tuple)):
            base_methods = [
                str(m).lower().strip() for m in base_methods_in if str(m).strip()
            ]
        else:
        ...
        base_methods = [
            m for m in base_methods if m in available_methods and m != "ensemble"
        ]
        ...
        params_in = (
            params.get("method_params")
            if isinstance(params.get("method_params"), dict)
            else {}
        )
        params_map = {
        ...
        for method_name in base_methods:
            ...
            try:
                ...
                continue
            if fc_arr.size < int(horizon) or not np.all(
                np.isfinite(fc_arr[: int(horizon)])
            ):
                _append_failure(
                    component_failures,
                    stage="component",
        ...
        if len(component_methods) != len(base_methods):
            keep_idx = [
                base_methods.index(method_name) for method_name in component_methods
            ]
        ...
        if effective_mode != "stacking":
            ...
            combined = np.full_like(
                component_forecasts[0], ensemble_intercept, dtype=float
            )
