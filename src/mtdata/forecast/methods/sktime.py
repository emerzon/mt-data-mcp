...
_SKTIME_IMPORT_ERROR = (
    "sktime is not installed; install it to enable sktime-based forecast methods."
)
...
class SktimeMethod(ForecastMethod):
    ...
    def forecast(
        ...
        **kwargs,
        ...
        if (
            not isinstance(y.index, pd.RangeIndex)
            and getattr(y.index, "freq", None) is None
        ):
            try:
                        ...
                        if isinstance(y.index, pd.RangeIndex):
                            ...
                            if freq is not None:
                                try:
                                    ...
                                    idx = pd.date_range(
                                        start=start, periods=horizon, freq=offset
                                    )
                                    X_future = pd.DataFrame(X_future, index=idx)
                                except Exception:
                                    pass
                            ...
                            try:
                                ...
                                if freq is not None:
                                    start = y.index[-1] + 1
                                    idx = pd.period_range(
                                        start=start, periods=horizon, freq=freq
                                    )
                                    X_future = pd.DataFrame(X_future, index=idx)
                            except Exception:
                                pass
...
class GenericSktimeMethod(SktimeMethod):
    ...
    PARAMS: List[Dict[str, Any]] = [
        ...
        },
        {
            ...
            "type": "dict",
            "description": "Constructor kwargs for estimator.",
        },
        {
            "name": "seasonality",
            "type": "int|null",
            "description": "Seasonal period (sp) if supported.",
        },
    ]
    ...
    def _get_estimator(self, seasonality: int, params: Dict[str, Any]):
        ...
        try:
            ...
            raise ValueError(
                f"Could not import sktime estimator '{estimator_path}': {e}"
            )
