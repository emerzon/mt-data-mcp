...
def _extract_forecast_values(
    Yf: Any, fh: int, method_name: str = "forecast"
) -> "np.ndarray":
    ...
    try:
        pred_candidates = [
            c for c in list(Yf.columns) if c not in ("unique_id", "ds", "y")
        ]
        if pred_candidates:
...
def _create_training_dataframes(
    series: np.ndarray,
    fh: int,
    ...
    if exog_used is not None and isinstance(exog_used, np.ndarray) and exog_used.size:
        ...
        if (
            ...
            and isinstance(exog_future, np.ndarray)
            and exog_future.size
        ):
            Xf_df = _pd.DataFrame(
                {
                    "unique_id": ["ts"] * int(fh),
                    "ds": _pd.RangeIndex(
                        start=int(len(series)), stop=int(len(series)) + int(fh)
                    ),
                }
            )
            for j, cname in enumerate(cols):
    ...
    *,
    ...
    with warnings.catch_warnings():
        ...
        if (
            ...
            and isinstance(exog_used, np.ndarray)
            and exog_used.size
            and supports_x
        ):
            # Build X_df and X_future for NF (newer API)
            X_df = pd.DataFrame(
                {"unique_id": ["ts"] * int(len(Y_df)), "ds": Y_df["ds"].values}
            )
            cols = [f"x{i}" for i in range(exog_used.shape[1])]
            for j, cname in enumerate(cols):
                X_df[cname] = exog_used[:, j]
            ...
            if (
                ...
                and isinstance(exog_future, np.ndarray)
                and exog_future.size
                and future_times is not None
            ):
                ds_f = pd.to_datetime(pd.Series(future_times), unit="s", utc=True)
                Xf_df = pd.DataFrame(
                    {
                        "unique_id": ["ts"] * int(len(ds_f)),
                        "ds": pd.Index(ds_f).to_pydatetime(),
                    }
                )
                for j, cname in enumerate(cols):
                    Xf_df[cname] = exog_future[:, j]
