...
def forecast_barrier_hit_probabilities(
    ...
    try:
        ...
        last_price_close, last_price, last_price_source, price_warning, price_error = (
            _resolve_reference_prices(
        ...
        if not _barrier_prices_are_valid(
            ...
            return {
                "error": "Resolved TP/SL barriers are invalid for the current price."
            }
        ...
        try:
            if method_key in ("mc_gbm", "mc_gbm_bb"):
                sim = _simulate_gbm_mc(
                    prices, horizon=horizon_val, n_sims=int(sims), seed=int(seed)
                )
                ...
                return {
                    "error": f"Unsupported method: {method}. Use 'mc_gbm', 'mc_gbm_bb', 'hmm_mc', 'garch', 'bootstrap', 'heston', 'jump_diffusion', or 'auto'"
                }
        ...
        if bb_enabled:
            ...
            if not np.isfinite(bb_sigma) or bb_sigma <= 0:
                ...
                bb_log_paths = np.concatenate(
                    [np.full((S, 1), log_s0), log_paths], axis=1
                )
        ...
        if (
            bb_enabled
            and bb_log_paths is not None
            and bb_uniform_tp is not None
            and bb_uniform_sl is not None
        ):
            tp_dir = "up" if dir_long else "down"
            sl_dir = "down" if dir_long else "up"
            tp_bridge = _brownian_bridge_hits(
                bb_log_paths,
                float(np.log(tp_price)),
                bb_sigma,
                direction=tp_dir,
                uniform=bb_uniform_tp,
            )
            sl_bridge = _brownian_bridge_hits(
                bb_log_paths,
                float(np.log(sl_price)),
                bb_sigma,
                direction=sl_dir,
                uniform=bb_uniform_sl,
            )
...
def forecast_barrier_closed_form(
    ...
    try:
        ...
        if mu is None or sigma is None:
            ...
            sigma_hat = (
                float(np.std(r, ddof=1)) * (365.0 * 24.0 * 3600.0 / tf_secs) ** 0.5
            )
            if mu is None:
                mu = mu_hat
            if sigma is None:
        ...
        if direction_norm == "short":
            ...
            prob = _gbm_upcross_prob(
                s0_inv, b_inv, float(inv_drift), sigma_val, float(T)
            )
        else:
            prob = _gbm_upcross_prob(
                s0, float(barrier), float(gbm_drift), sigma_val, float(T)
            )
        return {
            "success": True,
            "symbol": symbol,
