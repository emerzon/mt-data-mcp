...
from .constants import SERVICE_NAME  # re-export for CLI/tests
...
def _resolve_transport(default: str = "sse") -> tuple[str, Optional[str]]:
    ...
    runtime = load_mcp_runtime_settings(
        default_transport=cast(Literal["stdio", "sse", "streamable-http"], default)
    )
    mount_path = (
        runtime.mount_path
        if runtime.transport == "sse" and runtime.mount_path not in ("", "/")
        else None
    )
...
def main(
    *,
    ...
    load_environment()
    bootstrap_tools()
    runtime = runtime_settings or load_mcp_runtime_settings(
    ...
    if settings is not None:
        log_level = getattr(
            logging, str(getattr(settings, "log_level", "INFO")).upper(), logging.INFO
        )
    else:
    ...
    if run_fn is not None:
        transport_literal = cast(
            Literal["stdio", "sse", "streamable-http"], transport_name
        )
        run_fn(
            transport=transport_literal,
            mount_path=mount_path if transport_name == "sse" else None,
        )
