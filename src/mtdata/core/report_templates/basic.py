...
def _parse_formatted_output(output: str) -> Dict[str, Any]:
    ...
    try:
        ...
        while i < len(lines):
                    ...
                    if key == "levels":
                        ...
                        while i < len(lines):
                            ...
                            if "," in stripped_line and (
                                stripped_line.startswith("level,")
                                or all(
                                    c.isdigit() or c in ".,- " or c.isalpha()
                                    for c in stripped_line[:20]
                                )
                            ):
                        ...
                        while i < len(lines) and (
                            ...
                            if next_line.startswith(" ") and ":" in next_line:
                                ...
                                result[current_section][sub_key.strip()] = _parse_value(
                                    sub_val.strip()
                                )
                            i += 1
                        i -= 1  # Back up one
            ...
            # Standalone delimited table data
                ...
                while (
                    i < len(lines)
                    and "," in lines[i]
                    and not (lines[i].count(":") > lines[i].count(","))
                ):
        ...
        if not result:
            result = {
                "error": "Could not parse formatted output",
...
def _compute_tr(high: List[float], low: List[float], close: List[float]) -> List[float]:
    ...
    for i in range(n):
        ...
        low_val = low[i] if i < len(low) and low[i] is not None else prev_close
        c = close[i] if close[i] is not None else prev_close
        a = abs(h - low_val)
        b = abs(h - prev_close)
        d = abs(low_val - prev_close)
        tr_val = max(a, b, d)
        tr.append(tr_val)
        prev_close = c
    return tr
...
def _compute_compact_trend(rows: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    clean_low = [
        low_val if low_val is not None else c
        for low_val, c in zip(lows, clean_close)
    ]
...
def _explain_compact_trend(compact: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(s, list):
        out["slope_atr_score_5_20_60"] = [
            int(v) for v in s[:3] if isinstance(v, (int, float))
        ]
    if isinstance(r, list):
        out["fit_r2_pct_5_20_60"] = [
            int(v) for v in r[:3] if isinstance(v, (int, float))
        ]
    for key in ("v", "q", "h", "l"):
        val = compact.get(key)
...
def template_basic(
        ...
        if "pivot_multi" not in secs:
            ...
            for tfp in ["H4", "D1"]:
                ...
                if base_tf and tfp_upper == base_tf:
                    continue
                res = _get_raw_result(
                    _compute_pivot_points, symbol=symbol, timeframe=tfp
                )
    ...
    for hh in vol_horizons:
        ...
        for m in methods:
            ...
            try:
                ...
                if (
                    fv is not None and fv == fv and fv >= 0.0
                ):  # finite check (fv==fv filters NaN)
                    use_val = fv
            except Exception:
                use_val = None
            if use_val is None:
                ...
                psh = proxy_res.get("horizon_sigma_return") or proxy_res.get(
                    "horizon_sigma_price"
                )
                try:
    ...
    try:
        ...
        rmse_tol = 0.05
    min_dir_acc_raw = p.get(
        "backtest_min_directional_accuracy", p.get("backtest_min_accuracy")
    )
    try:
    ...
    if "error" in bt:
        ...
        try:
            ...
            ranking.sort(
                key=lambda x: (
                    float(x.get("avg_rmse") or 1e9),
                    -float(x.get("avg_directional_accuracy") or 0.0),
                )
            )
        except Exception:
            pass
        ...
        if min_dir_acc is not None:
            sec_bt["selection_criteria"]["min_directional_accuracy"] = float(
                min_dir_acc
            )
            sec_bt["selection_criteria"]["min_directional_accuracy_pct"] = float(
                min_dir_acc * 100.0
            )
    ...
    if best is not None:
        ...
        for method_name in candidate_methods:
            ...
            if "error" in fc:
                ...
                fallback_notes.append(
                    f"{method_name}: forecast error ({fc.get('error')})"
                )
                continue
        ...
        if selected_forecast is None and first_degenerate is not None:
            ...
            selected_stats = dict(
                stats_by_method.get(selected_method) or best_stats or {}
            )
        ...
        if selected_forecast is None:
            ...
            report["sections"]["forecast"] = {
                ...
                "last_observation_epoch": selected_forecast.get(
                ...
                "forecast_start_gap_bars": selected_forecast.get(
            ...
            if selected_method != best_name:
                ...
                report["sections"]["forecast"]["fallback_reason"] = (
                    "initial best method produced a degenerate forecast"
                )
        ...
        best_method_payload: Dict[str, Any] = {
            ...
            "stats": {
                ...
                "avg_directional_accuracy": selected_stats.get(
        ...
        try:
            ...
            if isfinite(sel_rmse):
                ...
                if selection_basis.get("best_rmse") is not None:
                    tol_limit = float(selection_basis["best_rmse"]) * (
                        1.0 + float(rmse_tol)
                    )
                    selection_basis["rmse_tolerance_limit"] = tol_limit
                    selection_basis["within_rmse_tolerance"] = bool(
                        sel_rmse <= tol_limit
                    )
        except Exception:
            pass
        ...
        if selected_forecast is not None and selected_method != best_name:
            ...
            best_method_payload["selection_warning"] = (
                "Initial best method forecast was degenerate; fallback applied."
            )
        if fallback_notes:
    ...
    if "error" in grid_long and "error" in grid_short:
        sec_bar = {
            "error": grid_long.get("error")
            or grid_short.get("error")
            or "Barrier optimization failed"
        }
    else:
        if "error" not in grid_long:
            sec_bar["long"] = summarize_barrier_grid(
                grid_long, top_k=int(p.get("top_k", 5))
            )
        else:
            sec_bar["long"] = {"error": grid_long.get("error")}
        if "error" not in grid_short:
            sec_bar["short"] = summarize_barrier_grid(
                grid_short, top_k=int(p.get("top_k", 5))
            )
        else:
