...
def template_advanced(
    ...
    p = dict(params or {})
    bocpd = _get_raw_result(
        regime_detect,
        symbol=symbol,
        timeframe=tf,
        limit=int(p.get("regime_limit", 1500)),
        method="bocpd",
        threshold=float(p.get("cp_threshold", 0.6)),
        output="summary",
        lookback=int(p.get("regime_lookback", 300)),
    )
    hmm = _get_raw_result(
        regime_detect,
        symbol=symbol,
        ...
        limit=int(p.get("regime_limit", 1500)),
        method="hmm",
        params={"n_states": int(p.get("hmm_states", 3))},
        output="compact",
        lookback=int(p.get("regime_lookback", 300)),
    )
