...
def pick_best_forecast_method(
    ...
    try:
        ...
        if min_da is not None:
            entries = [
                e for e in entries if e[3] is not None and float(e[3]) >= float(min_da)
            ]
        if not entries:
            return None
    ...
    try:
        ...
        if not top and isinstance(grid.get("results"), list):
            ...
            try:
                items = sorted(
                    items,
                    key=lambda x: float(x.get("score", x.get("edge", -1e9))),
                    reverse=True,
                )
            except Exception:
                pass
            ...
            try:
                ...
                if ev_val is not None and edge_val is not None:
                    ...
                    if (ev_f > 0.0 and edge_f < 0.0) or (ev_f < 0.0 and edge_f > 0.0):
                        ...
                        best_out["ev_edge_conflict_reason"] = (
                ...
                trimmed.append(
                    {
                        "tp": it.get("tp"),
                        "sl": it.get("sl"),
                        "tp_price": it.get("tp_price"),
                        "sl_price": it.get("sl_price"),
                        "edge": it.get("edge"),
                        "kelly": it.get("kelly"),
                        "ev": it.get("ev"),
                        "prob_tp_first": it.get("prob_tp_first"),
                        "prob_sl_first": it.get("prob_sl_first"),
                        "prob_no_hit": it.get("prob_no_hit"),
                    }
                )
...
def merge_params(
    base: Optional[Dict[str, Any]], extra: Dict[str, Any], override: bool = False
) -> Dict[str, Any]:
    p = dict(base or {})
    ...
    try:
        ...
        if isinstance(dom, dict) and dom.get("success"):
            ...
            if t == "tick_data":
                ...
                try:
                    total_buy_vol = (
                        float(sum(float(b.get("volume") or 0.0) for b in buys))
                        if buys
                        else None
                    )
                except Exception:
                ...
                try:
                    total_sell_vol = (
                        float(sum(float(s.get("volume") or 0.0) for s in sells))
                        if sells
                        else None
                    )
                except Exception:
        ...
        if tick_size is not None and tick_size > 0:
            pip_size = tick_size
            if math.isclose(
                tick_size, 0.00001, rel_tol=0.0, abs_tol=1e-12
            ) or math.isclose(
                tick_size,
                0.001,
                rel_tol=0.0,
        ...
        if tick_size and spread is not None:
            try:
                spread_ticks = (
                    float(spread) / float(tick_size) if tick_size > 0 else None
                )
            except Exception:
                spread_ticks = None
        spread_pips = None
...
def apply_market_gates(
    section: Dict[str, Any], params: Dict[str, Any]
) -> Dict[str, Any]:
    gate = {}
    try:
...
def attach_multi_timeframes(
    ...
    for tf in extra_timeframes or []:
        ...
        if snap:
            ...
            if not isinstance(snap_for_contexts, dict) or any(
                v is not None for v in snap_for_contexts.values()
            ):
