...
def server_shared_defs(shared_enums: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _build_defs() -> None:
        defs.update(
            {
                "OhlcvChar": {
                    "type": "string",
                    "enum": ["O", "H", "L", "C", "V"],
                    "description": "OHLCV column code",
                },
                "DenoiseMethod": {
                    "type": "string",
                    "enum": list(shared_enums.get("DENOISE_METHODS", [])),
                },
                "SimplifyMode": {
                    "type": "string",
                    "enum": list(shared_enums.get("SIMPLIFY_MODES", [])),
                },
                "SimplifyMethod": {
                    "type": "string",
                    "enum": list(shared_enums.get("SIMPLIFY_METHODS", [])),
                },
                "EncodeSchema": {"type": "string", "enum": ["envelope", "delta"]},
                "SymbolicSchema": {"type": "string", "enum": ["sax"]},
                "PivotMethod": {
                    "type": "string",
                    "enum": list(shared_enums.get("PIVOT_METHODS", [])),
                },
                "ForecastMethod": {
                    "type": "string",
                    "enum": list(shared_enums.get("FORECAST_METHODS", [])),
                },
                "QuantitySpec": {
                    "type": "string",
                    "enum": ["price", "return", "volatility"],
                },
                "VolatilityMethod": {
                    "type": "string",
                    "enum": [
        ...
        if shared_enums.get("CATEGORY_CHOICES"):
            defs["IndicatorCategory"] = {
                "type": "string",
                "enum": list(shared_enums["CATEGORY_CHOICES"]),
            }
        if shared_enums.get("INDICATOR_NAME_CHOICES"):
            defs["IndicatorName"] = {
                "type": "string",
                "enum": list(shared_enums["INDICATOR_NAME_CHOICES"]),
            }
...
def _patch_data_fetch_candles_schema(schema: Dict[str, Any]) -> None:
    ...
    _set_ref(params, required_params, "denoise", "#/$defs/DenoiseSpec", allow_null=True)
    _set_ref(
        params, required_params, "simplify", "#/$defs/SimplifySpec", allow_null=True
    )
        ...
        for name, obj in list(registry.items()):
            ...
            def _update_defs() -> None:
                if "$defs" not in schema:
                    schema["$defs"] = {}
                schema["$defs"].update(
                    {k: v for k, v in shared_defs.items() if k not in schema["$defs"]}
                )
                schema["$defs"].update(
                    {
                        k: v
                        for k, v in _complex_defs().items()
                        if k not in schema["$defs"]
                    }
                )
            ...
            _safe_schema_op(
                f"set_obj_schema:{name}", lambda: setattr(obj, "schema", schema)
            )
            _safe_schema_op(
                f"set_func_schema:{name}", lambda: setattr(func, "schema", schema)
            )
    except Exception:
        pass
