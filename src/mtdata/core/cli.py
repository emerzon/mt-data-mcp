...
def _top_pattern_name(row: Dict[str, Any]) -> Optional[str]:
...
def _compact_patterns_result_for_cli(result: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(result, dict) or "error" in result:
        return result

    rows: List[Dict[str, Any]] = []
    if isinstance(result.get("patterns"), list):
        rows = [row for row in result.get("patterns", []) if isinstance(row, dict)]
    ...
    def _sort_key(item: Tuple[int, Dict[str, Any]]) -> Tuple[float, float, int]:
        ...
        return (
            end_idx if end_idx is not None else float(idx),
            confidence,
            idx,
        )
    ...
    for row in preview_rows:
        ...
        if row.get("timeframe") not in (None, ""):
            compact_row["timeframe"] = row.get("timeframe")
        label = _top_pattern_name(row)
        if label:
            compact_row["pattern"] = label
        for key in ("time", "end_date", "start_date", "status", "confidence"):
            value = row.get(key)
            if value not in (None, ""):
                compact_row[key] = value
        if not compact_row:
            compact_row = dict(row)
        recent_rows.append(compact_row)

    strongest_pattern: Optional[Dict[str, Any]] = None
    if preview_rows:
        best = max(preview_rows, key=lambda row: (_safe_float(row.get("confidence")) or 0.0))
        best_name = _top_pattern_name(best)
        strongest_pattern = {}
        if best_name:
            strongest_pattern["pattern"] = best_name
    ...
    try:
        ...
        total = len(rows)
    summary["more_patterns"] = max(0, total - len(recent_rows))
    ...
    for key in ("lookback", "candles", "engine", "scanned_timeframes"):
        value = result.get(key)
        if value not in (None, "", []):
            compact[key] = value
    for key in ("engine_findings", "engine_errors", "findings"):
        value = result.get(key)
        if value not in (None, "", [], {}):
            compact[key] = value
...
def _compact_regime_result_for_cli(result: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(result, dict) or "error" in result:
        return result
    ...
    rows = [row for row in regimes if isinstance(row, dict)]
    if not rows:
        return result

    preview_limit = 5
    recent_rows = rows[-preview_limit:]
    current = recent_rows[-1]
    previous = recent_rows[-2] if len(recent_rows) >= 2 else None
    bars_total = 0
    for row in rows:
        try:
            bars_total += int(row.get("bars") or 0)
        except Exception:
            continue

    summary: Dict[str, Any] = {
        "regime_changes": max(0, len(rows) - 1),
        "lookback_bars": bars_total,
        "current_regime": current.get("regime"),
        "current_regime_started": current.get("start"),
        "current_regime_bars": current.get("bars"),
        "last_transition": current.get("start") if previous is not None else None,
    }
    if previous is not None:
        summary["previous_regime"] = previous.get("regime")
    if current.get("avg_conf") not in (None, ""):
        summary["current_regime_confidence"] = current.get("avg_conf")
    ...
    compact: Dict[str, Any] = {
        ...
        "summary": summary,
        "recent_regimes": recent_rows,
    }
    if isinstance(method_summary, dict) and method_summary:
        compact["method_summary"] = method_summary
    if result.get("params_used"):
        compact["params_used"] = result.get("params_used")
    compact["more_regimes"] = max(0, len(rows) - len(recent_rows))
    return compact


def _compact_cli_result(result: Any, *, cmd_name: str, verbose: bool) -> Any:
    if verbose or not isinstance(result, dict):
        return result
    if cmd_name == "patterns_detect":
        return _compact_patterns_result_for_cli(result)
    if cmd_name == "regime_detect":
        return _compact_regime_result_for_cli(result)
    return result
...
def _format_result_for_cli(result: Any, *, fmt: str, verbose: bool, cmd_name: str) -> str:
    ...
    result = _compact_cli_result(result, cmd_name=cmd_name, verbose=verbose)
...
_COMMAND_USAGE_EXAMPLES: Dict[str, Tuple[str, Optional[str]]] = {
    "patterns_detect": (
        ...
        "python cli.py patterns_detect BTCUSD --timeframe H1 --mode classic --limit 300 --verbose",
    ),
