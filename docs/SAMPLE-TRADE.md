...
| **Result** | JSON with a 7x9 grid (63 combos). Each candidate includes TP/SL hit probabilities, edge, Kelly, EV (conditional and unconditional), plus median time-to-hit stats. | **Interpretation** <br>- Use `objective=edge` for setups where TP-first odds are the priority; switch to `kelly`, `ev`, or unconditional metrics when payoff asymmetry matters more. <br>- Wider stops now show up in the default grid, so you can choose conservative ratios without redefining the search space. |
