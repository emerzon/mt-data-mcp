...
from src.mtdata.core.trading import trading_positions_get, trading_pending_get
...
class TestMergedTools(unittest.TestCase):
    def test_trading_positions_get(self):
        ...
        # Test default
        res = trading_positions_get(__cli_raw=True)
        self.assertIsInstance(res, list)
        
        # Test with symbol
        trading_positions_get(symbol="EURUSD", __cli_raw=True)
        mt5.positions_get.assert_called_with(symbol="EURUSD")
        
        # Test with ticket
        trading_positions_get(ticket=123, __cli_raw=True)
        mt5.positions_get.assert_called_with(ticket=123)

    def test_trading_pending_get(self):
        mt5.orders_get.return_value = None
        
        trading_pending_get(__cli_raw=True)
        
        trading_pending_get(symbol="EURUSD", __cli_raw=True)
        mt5.orders_get.assert_called_with(symbol="EURUSD")
        
        trading_pending_get(ticket=123, __cli_raw=True)
    ...
    def test_trading_positions_close(self):
        # Mock positions
        mock_pos = MagicMock()
        mock_pos.ticket = 123
        ...
        from src.mtdata.core.trading import trading_positions_close
        
        # Test close by ticket
        trading_positions_close(ticket=123, __cli_raw=True)
        mt5.positions_get.assert_called_with(ticket=123)
        
        # Test close by symbol
        trading_positions_close(symbol="EURUSD", __cli_raw=True)
        mt5.positions_get.assert_called_with(symbol="EURUSD")
        
        # Test close all
        trading_positions_close(__cli_raw=True)
        mt5.positions_get.assert_called_with()

    def test_trading_pending_cancel(self):
        # Mock orders
        ...
        from src.mtdata.core.trading import trading_pending_cancel
        
        # Test cancel by ticket
        trading_pending_cancel(ticket=456, __cli_raw=True)
        mt5.orders_get.assert_called_with(ticket=456)
        
        # Test cancel by symbol
        trading_pending_cancel(symbol="EURUSD", __cli_raw=True)
        mt5.orders_get.assert_called_with(symbol="EURUSD")
        
        # Test cancel all
        trading_pending_cancel(__cli_raw=True)
