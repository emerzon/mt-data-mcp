...
def test_denoise_series_tv_kalman():
    ...
    kalman = _denoise_series(
        s, method="kalman", params={"process_var": "auto", "measurement_var": "auto"}
    )
    assert len(tv) == len(s)
...
def test_denoise_series_butterworth():
    ...
    result = _denoise_series(
        s, method="butterworth", params={"cutoff": 0.2, "order": 3}
    )
    assert len(result) == len(s)
    assert np.isfinite(result.values).all()
...
def test_denoise_series_hampel_bilateral():
    ...
    bilateral = _denoise_series(
        s, method="bilateral", params={"sigma_s": 2.0, "sigma_r": 3.0}
    )
...
def test_denoise_series_wavelet_packet():
    ...
    s = pd.Series(np.sin(np.linspace(0, 2 * np.pi, 64)))
    result = _denoise_series(
        s, method="wavelet_packet", params={"wavelet": "db2", "level": 2}
    )
    assert len(result) == len(s)
    assert np.isfinite(result.values).all()
...
def test_denoise_series_adaptive_beta():
    ...
    rls = _denoise_series(
        s, method="rls", params={"order": 4, "lambda_": 0.98, "delta": 1.0}
    )
    beta = _denoise_series(
        s, method="beta", params={"window": 7, "beta": 1.3, "n_iter": 5}
    )
...
def test_denoise_series_vmd():
    ...
    s = pd.Series(np.sin(np.linspace(0, 2 * np.pi, 64)))
    result = _denoise_series(
        s, method="vmd", params={"k": 3, "alpha": 1000.0, "drop_modes": [-1]}
    )
    assert len(result) == len(s)
    assert np.isfinite(result.values).all()
