...
def test_forecast_barrier_prob_request_rejects_multiple_tp_unit_families():
    with pytest.raises(ValidationError, match="Provide only one take-profit unit family: tp_abs, tp_pct, tp_pips"):
...
def test_forecast_barrier_prob_request_rejects_multiple_sl_unit_families():
    with pytest.raises(ValidationError, match="Provide only one stop-loss unit family: sl_abs, sl_pct, sl_pips"):
