...
class TestBarrierStats(unittest.TestCase):
    ...
    def test_bootstrap_metric_uncertainty_uses_entry_price(self):
        ...
        with patch(
            "mtdata.forecast.barrier_stats.np.random.RandomState", FakeRandomState
        ):
            result = bootstrap_metric_uncertainty(
                paths=paths,
                tp_trigger=110.0,
...
class TestBarrierOptimizationWithStats(
    _BarrierOptimizationPatchMixin, unittest.TestCase
):
    ...
    def test_barrier_optimize_cross_seed_stability_replays_best_candidate(self):
        ...
        def fake_sim(prices, horizon, n_sims, seed, **kwargs):
            ...
            paths = np.vstack(
                [
                    np.repeat(win_path, win_count, axis=0),
                    np.repeat(loss_path, loss_count, axis=0),
                ]
            )
    ...
    def test_barrier_optimize_short_convergence_uses_short_threshold(self):
        ...
        def fake_convergence(
            cumulative_successes, cumulative_trials, window_size, threshold
        ):
            ...
            return {
                ...
                "current_estimate": float(
                    cumulative_successes[-1] / cumulative_trials[-1]
                ),
                "window_mean": 0.0,
                "window_std": 0.0,
                "max_change": 0.0,
        ...
        with patch(f"{_BARRIER_OPT_ROOT}._simulate_gbm_mc", side_effect=fake_sim):
            with patch(
                "mtdata.forecast.barriers_optimization._mc_convergence",
